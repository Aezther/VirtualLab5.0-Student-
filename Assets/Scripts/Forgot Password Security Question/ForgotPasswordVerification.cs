...
public class ForgotPasswordVerification : MonoBehaviour
{
    ...
    public void ValidateID (){
        using (IDbConnection dbConnection = new SqliteConnection(connectionString)) {
            ...
            using (IDbCommand dbCmd = dbConnection.CreateCommand()) {
                ...
                sqlQuery = "SELECT StudentID FROM SecurityQuestionsStudentsTBL";
                    ...
                    while (reader.Read()) {
                        ...
                        if (listofTeacherIDwithSQuestions == TeacherID.text){
                            ...
                            if(string.IsNullOrEmpty(TeacherID.text)){
                                TeacherIDError.text = "StudentID Inputfield is empty.";
                            }
    ...
    public void GetTeacherID(){
        using (IDbConnection dbConnection = new SqliteConnection(connectionString)) {
            ...
            using (IDbCommand dbCmd = dbConnection.CreateCommand()) {
                //start
                sqlQuery = "SELECT Question FROM SecurityQuestionsStudentsTBL WHERE StudentID = '"+TeacherID.text+"';";
                dbCmd.CommandText = sqlQuery;
                using (IDataReader reader = dbCmd.ExecuteReader()) {
    ...
    public void CheckAnswer(){
        using (IDbConnection dbConnection = new SqliteConnection(connectionString)) {
            ...
            using (IDbCommand dbCmd = dbConnection.CreateCommand()) {
                //start
                sqlQuery = "SELECT Question, Answer FROM SecurityQuestionsStudentsTBL WHERE StudentID = '"+TeacherID.text+"';";
                dbCmd.CommandText = sqlQuery;
                using (IDataReader reader = dbCmd.ExecuteReader()) {
                    while (reader.Read()) {
                        string Question = reader.GetString(0);
    ...
    public void VerifyNewPassword(){
        using (IDbConnection dbConnection = new SqliteConnection(connectionString)) {
            ...
            using (IDbCommand dbCmd = dbConnection.CreateCommand()) {
                ...
                sqlQuery = "UPDATE StudentsTBL SET Password = '"+SetNewPassword.text+"' WHERE StudentID = '"+TeacherID.text+"';";
                dbCmd.CommandText = sqlQuery;
                dbCmd.ExecuteNonQuery();
                //end
            }
