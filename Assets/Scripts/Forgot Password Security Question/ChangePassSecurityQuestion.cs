    ...
    public void CurrentPassword() {
        ...
        using (IDbConnection dbConnection = new SqliteConnection(connectionString)) {
            ...
            using (IDbCommand dbCmd = dbConnection.CreateCommand()) {
                //start
                //sqlQuery = "SELECT Password FROM TeachersTBL WHERE TeacherID = '"+ userInfo.StudentID+ "';";
                sqlQuery = "SELECT Password FROM StudentsTBL WHERE StudentID = '"+ userInfo.StudentID+ "';";
                ...
                using (IDataReader reader = dbCmd.ExecuteReader()) {
                    ...
                    reader.Close();
                            ...
                            (!string.IsNullOrEmpty(IFConfirmNewPassword.text) && IFNewPassword.text == IFConfirmNewPassword.text)) {
                                //InsertNewPassinDB();
                                //sqlQuery = "UPDATE TeachersTBL SET Password = '"+IFNewPassword.text+"' WHERE TeacherID = '"+userInfo.StudentID+"';";
                                sqlQuery = "UPDATE StudentsTBL SET Password = '"+IFNewPassword.text+"' WHERE StudentID = '"+userInfo.StudentID+"';";
                                dbCmd.CommandText = sqlQuery;
                                dbCmd.ExecuteNonQuery();
